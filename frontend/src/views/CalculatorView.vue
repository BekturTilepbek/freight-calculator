...
async function calculate() {
  ...
  error.value = null
  try {
    const { data } = await axios.post('/api/v1/calculations/estimate', form)
    result.value = data
  } catch (e) {
        ...
        <template #content>
          ...
          >
            ...
            <p class="text-surface-500 max-w-sm">
              ...
              чтобы увидеть детализацию стоимости и маржу.
            </p>
          </div>
          ...
          <!-- Ошибка -->
          <Message v-if="error" severity="error" :closable="false" class="m-0">
            {{ error }}
          </Message>
          ...
          <div v-if="result && !loading" class="space-y-5">
            ...
            <!-- Детализация -->
            <div>
              <h3 class="font-semibold text-sm uppercase tracking-wide text-surface-500 mb-3">
                Детализация
              </h3>
              <div class="space-y-2">
                <div
                ...
                >
                  <div>
                    <div class="font-medium text-surface-900 dark:text-surface-0">{{ item.name }}</div>
                    <div v-if="item.description" class="text-xs text-surface-500 mt-0.5">
                      {{ item.description }}
                    </div>
                  </div>
                  <div class="font-semibold tabular-nums">{{ fmt(item.amount) }}</div>
                </div>
              </div>
            </div>
          </div>
