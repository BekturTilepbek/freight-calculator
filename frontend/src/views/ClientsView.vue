...
const form = reactive({ name: '', contact_person: '', email: '', phone: '' })

async function load() {
  loading.value = true
  try {
    items.value = await clientsApi.list()
  } finally {
    loading.value = false
  }
}
...
function openCreate() {
  ...
  Object.assign(form, { name: '', contact_person: '', email: '', phone: '' })
...
async function save() {
  if (!form.name?.trim()) {
    toast.add({ severity: 'warn', summary: 'Заполните имя клиента', life: 2000 })
    return
  }
  saving.value = true
  try {
    ...
    if (editingId.value) {
      ...
      toast.add({ severity: 'success', summary: 'Клиент обновлен', life: 2000 })
    } else {
      await clientsApi.create(payload)
      toast.add({ severity: 'success', summary: 'Клиент создан', life: 2000 })
    }
...
function confirmDelete(row) {
  confirm.require({
    message: `Удалить клиента «${row.name}»?`,
...
<template>
  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <h2 class="text-sm text-surface-500">
        Всего клиентов: <span class="font-semibold text-surface-900 dark:text-surface-0">{{ items.length }}</span>
      </h2>
      <Button label="Новый клиент" icon="pi pi-plus" @click="openCreate" />
    </div>
    ...
    <Card>
      <template #content>
        ...
        >
          ...
          <Column field="contact_person" header="Контактное лицо">
            <template #body="{ data }">
              {{ data.contact_person || '—' }}
            </template>
          </Column>
  ...
  <Dialog
    ...
    modal
    :header="editingId ? 'Редактировать клиента' : 'Новый клиент'"
    :style="{ width: '500px' }"
    :closable="!saving"
  >
    <div class="space-y-4">
      <div>
        ...
        <InputText v-model="form.name" placeholder="Walmart Logistics" class="w-full" />
      </div>
      <div>
      ...
      <div>
        ...
        <InputText v-model="form.email" type="email" placeholder="contact@company.com" class="w-full" />
      </div>
      <div>
